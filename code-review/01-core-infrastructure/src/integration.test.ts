...
describe("Integration: End-to-End Command Handlers", () => {
  ...
  it("review-pr flow with mocked clients completes successfully", async () => {
    // Import the real reviewPR (not mocked) — it uses stub agents via shared.ts
    // The github client mock prevents real API calls
    const { runReviewPipeline } = await import("./commands/shared.js");
